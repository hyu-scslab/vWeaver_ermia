...
inline local_vector<T, N, A>::~local_vector() {
  for (T* it = r_.first_; it != r_.last_; ++it) r_.destroy(it);
...
inline void local_vector<T, N, A>::clear() {
  for (auto it = r_.first_; it != r_.last_; ++it) r_.destroy(it);
  r_.last_ = r_.first_;
}
...
inline void local_vector<T, N, A>::resize(size_type n, value_type v) {
  ...
  auto xt = r_.last_;
  r_.last_ = it;
  for (; it < xt; ++it) r_.destroy(it);
  for (; xt < it; ++xt) r_.construct(xt, v);
}

template <typename T, int N, typename A>
local_vector<T, N, A>& local_vector<T, N, A>::operator=(
    const local_vector<T, N, A>& x) {
  if (&x != this) {
    clear();
    if (capacity() < x.capacity()) grow(x.capacity());
...
T* local_vector<T, N, A>::erase(iterator first, iterator last) {
  if (first != last) {
    ...
    for (; last != xend; ++it, ++last) *it = std::move(*last);
    r_.last_ = it;
    for (; it != xend; ++it) r_.destroy(it);
  }
  return first;
}
